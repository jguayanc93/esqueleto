...
const {cliente} = require('../../id_nombres/lista')
...
async function bcli_codi(req,res,next) {
    try{
        ...
        const tercera_llamada= asignador_identificadores(segunda_llamada[0],cliente,1);
...
function obtenerpromesa_consulta1(conexion,req,next){
    return new Promise((resolve,reject)=>query_cliente(resolve,reject,conexion,req,next))
}


let query_cliente = (resolve,reject,conexion,req,next)=>{
    ...
    let sp_sql="select codcli,ruccli,nomcli,tipocl,dbo.cliente_hp_api(codcli) from mst01cli where estado=1 AND codcli=@codcli";
